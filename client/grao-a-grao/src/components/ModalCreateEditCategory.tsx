...
export type ModalCreateEditCategoryProps = {
    ...
    handleCreate: (newCategory: CreateCategoryInput) => void;
    handleEdit: (toUpdateCategory: UpdateCategoryInput) => void;
}
export default function ModalCreateEditCategory({
    ...
    const [description, setDescription] = useState<string>(editCategory ? editCategory.description : "");
                            ...
                            <Flex className="sm:mt-6 mb-4 sm:mb-0" justify={"start"} align={"center"}>
                                {isModalEdit ?
                                    (<Button size={"3"} onClick={
                                        (ev) => {
                                            ev.preventDefault();
                                            handleEdit({ id: editCategory!.id, description });
                                        }}>Editar</Button>)
                                    : isModalCreate ? 
                                    (<Button size={"3"} onClick={
                                        (ev) => {
